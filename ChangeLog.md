...
* Function `TRACEBACK-TO-STRING` was removed and
  replaced with [`log4cl-extras/error:print-backtrace`][d08a] which is now
...
* Fixed [`with-log-unhandled`][a7e2] for cases when some function argument's print-object signaled the error.

Because of this nasty error, sometimes [`with-log-unhandled`][a7e2] didn't log "Unandled error".

<a id="added"></a>

### Added

* Now [`log4cl-extras/error:with-log-unhandled`][a7e2] macro accepts key argument `DEPTH` which is 10 by default.

This argument can be overriden by setting [`log4cl-extras/error:*max-traceback-depth*`][29d2].

* Also another variable [`log4cl-extras/error:*max-call-length*`][8180] can be set to control
...
Now [`log4cl-extras/config:setup`][c848] sets appender into a mode when it prints log in a human
...
* Previously, macros [`log4cl-extras/error:with-log-unhandled`][a7e2] catched every signal,
...
* Added ability to log tracebacks using [`log4cl-extras/error:with-log-unhandled`][a7e2].
...
[8180]: index.html#x-28LOG4CL-EXTRAS-2FERROR-3A-2AMAX-CALL-LENGTH-2A-20-28VARIABLE-29-29
[29d2]: index.html#x-28LOG4CL-EXTRAS-2FERROR-3A-2AMAX-TRACEBACK-DEPTH-2A-20-28VARIABLE-29-29
